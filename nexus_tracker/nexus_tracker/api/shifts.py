...
def get_employee_route(employee,date):
    pings=frappe.get_all("FT Location Ping",filters={"employee":employee,"recorded_at":[">=",date]},
    ...
    visits=frappe.get_all("FT Client Visit",filters={"employee":employee,"checked_in_at":[">=",date]},
...
def get_daily_report(date=None,branch=None,department=None,employee=None):
    ...
    if department: f["department"]=department
    if employee: f["name"]=employee
    emps=frappe.get_all("FT Employee",filters=f,fields=["name","employee_name","department","branch"])
    report=[]
    for emp in emps:
