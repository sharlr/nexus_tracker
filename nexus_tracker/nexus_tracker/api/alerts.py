...
def check_inactive_employees():
    ...
    for shift in frappe.get_all("FT Shift", filters={"is_active": 1}, fields=["name", "employee"]):
        last = frappe.db.get_value("FT Location Ping",
            {"employee": shift.employee, "shift": shift.name},
            "recorded_at", order_by="recorded_at desc")
        if last and last < threshold:
            if not frappe.db.exists("FT Alert", {"employee": shift.employee,
                "alert_type": "Inactivity", "is_read": 0,
                "created_at": [">", add_to_date(now_datetime(), hours=-2)]}):
...
def check_low_battery():
    pings = frappe.db.sql("""
        SELECT DISTINCT employee, battery_level FROM `tabFT Location Ping`
        WHERE battery_level <= 15 AND battery_level IS NOT NULL AND recorded_at >= %s
    """, add_to_date(now_datetime(), minutes=-20), as_dict=True)
    for p in pings:
        if not frappe.db.exists("FT Alert", {"employee": p.employee,
            "alert_type": "Low Battery", "is_read": 0,
            "created_at": [">", add_to_date(now_datetime(), hours=-1)]}):
